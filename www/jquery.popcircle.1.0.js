    ...
    $.popcircle = function(selector, settings){
       var config = {
        ...
        $.each( li, function( key, value ) {
        	...
        	if(con_left==left&&con_top==top)
        	{
        		...
        		switch (config.type)
        		{
	        		case 'full':
			        		...
			        		var x=parseInt(con_left)+((parseInt(trig_width)+spacing)*Math.cos(rd));
			        		var y=parseInt(con_top)+((parseInt(trig_width)+spacing)*Math.sin(rd));
        	...
        	{
                var elt =$(this) ;
	        	$(this).animate({ top: con_top,left:con_left }, {duration: config.time,easing: config.ease, complete: function () { elt.css ('display', 'none') ; }});
        	}
